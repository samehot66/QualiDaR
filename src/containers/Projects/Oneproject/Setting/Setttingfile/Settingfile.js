...
const Setfile = (props) => {
    ...
    useEffect(() => {
        ...
        axios.get(config.URL + '/api/files/topic', data, axiosConfig,{ cancelToken: source.token})
            .then((res) => {
    ...
    const addFileHandler= async (pdfid)=>{
      ...
      await axios.post(config.URL + '/api/files/topic', data, axiosConfig)
            .then((res) => {
    ...
    const removeFileHandler= async (pdfid)=>{
      ...
      await axios.delete(config.URL + '/api/files/topic', data, axiosConfig)
            .then((res) => {
        ...
        isauth ?
            <Auxi>
                        ...
                        <div className={["card card-primary", classes.Box].join(' ')} >
                            ...
                            <div className="card-body p-0 " style={{ overflow: "auto" }}>
                                <table className="table m-0 text-nowrap" style={{ overflow: "scroll" }}>
                                    ...
                                    {file.map(f => (
                                    <tr key={f.pdfid}>
                                               <td style={{ color: "#007bff" }}> <i className="fa fa-fw  fa-file-pdf" ></i> {f.filename}</td>
                                               <td><i id={f.pdfid}  className="fa fa-fw fa-plus-square" style={{ fontSize: "18px", color:"#007bff"  }} data-toggle="tooltip" data-placement="top" title="Add" onClick={(e)=>addFileHandler(e.target.id)}></i></td>
                                         </tr>
                                    ))}
                        ...
                        <div className={["card card-info", classes.Box].join(' ')}>
                            <div className="card-header border-transparent " style={{ padding: "0.2rem 1rem" }}>
                                <h3 className="card-title">File(s) in this project
                            ...
                            <div className="card-body p-0 " style={{ overflow: "auto" }}>
                                <div >
                                    ...
                                    <tr key={f.pdfid}>
                                               <td style={{ color: "#007bff" }}> <i  className="fa fa-fw  fa-file-pdf" ></i> {f.filename}</td>
                                               <td><i id={f.pdfid} className="fa fa-fw fa-minus-square" style={{ fontSize: "18px", color:"#dc3545" }} data-toggle="tooltip" data-placement="top" title="Remove" onClick={(e)=>removeFileHandler(e.target.id)}></i></td>
                                         </tr>
                                    ))}
